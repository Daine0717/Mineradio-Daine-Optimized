...
## Daine优化版2.1.2（2026-07-15）

- 每日歌词改为基于用户歌单与收听偏好的个性化推荐：优先选择符合常听歌手、歌单来源和近期偏好、但较久未播放的歌曲，并按天缓存结果；每日推荐功能保持独立，不受影响。
- 优化副歌与评论展示：优先识别歌词中的副歌标记或重复段落，缓存提取结果；移除底部波形条，改为展示该歌曲点赞数最高的评论。
- 每日歌词右上角改为专辑封面 CD：封面以 24 秒一圈旋转，并沿 1 点钟至 7 点钟方向的轴进行 ±15°、8 秒周期的轻微 3D 扭动。
- Home 页“所有歌单”使用独立的叠层歌单图标，不再与“我喜欢”或首个歌单封面重复。
- 重做播放列表快捷操作：移除右上角全局随机按钮，在每个歌单信息卡片中加入“随机播放”，一次载入完整歌单、打乱队列并立即播放；普通播放仍保持原顺序。
- 将歌单信息区内无效的“回到顶部”按钮替换为播放列表底部的玻璃胶囊按钮；顶部可见时隐藏，向下滚动离开顶部后浮现。
- 修复从 Home 打开播放队列后瞬间缩回的问题：按钮展开后需再次点击关闭，或等待鼠标首次进入列表后才启用自动收回；播放页镜头改为依据列表真实展开状态切换。
- 从根源修复卡片悬停闪烁：Home 卡片、快捷入口和歌单卡片统一使用固定透明命中层与 `pointerenter` / `pointerleave`，仅移动内部视觉层，避免上浮后丢失鼠标命中。
- 复用舞台歌词与视觉引擎中的颜色计算对象，减少逐帧临时对象与垃圾回收抖动，提升动画稳定性。

## Daine优化版2.1.1（2026-07-13）

- 新增极简模式：仅在播放页账号信息下方显示圆形液态玻璃“简”入口，点击后将当前大窗口平滑切换为 `900 × 150` 歌词条，并与原大窗口底边对齐。
- 极简模式采用中性半透明玻璃窗口，仅轻微压暗和模糊屏幕后方画面；移除原有蓝色底色，并基于“星河”制作低密度、低染色的极简专用视觉。
- 极简模式下固定使用专用“星河”视觉，切歌、换封面和预设同步不会再恢复大屏预设；退出后完整恢复进入前的视觉设置。
- 歌词在极简窗口内保持上下居中并占据约 80% 横向空间；顶部加入上一首、播放/暂停和下一首基础控制器。
- 新增极简窗口屏幕边缘约束，拖拽到当前屏幕工作区边缘后不再越界。
- 重做极简切换过渡：使用开屏 Logo 与“极简”文字动画承接窗口变化，进入前静置 `400ms`，文字动画持续 `1.2s`；退出时先定格 `400ms`，再以反向扩散动画淡出。
- 移除“极简”文字的青色发光，只保留文字本体和中性可读性阴影；切换期间隐藏音乐控制器、退出按钮和关闭按钮，避免动画画面割裂。
- 优化窗口变化性能：resize 使用 `requestAnimationFrame` 节流和停止后 debounce，变化期间暂停动画并隐藏复杂组件，停止后统一刷新布局；真实窗口尺寸改为过渡完成后一次性应用，减少掉帧、拖影和歌词闪烁。
- 极简模式右上角新增与关闭按钮同样风格的“简”退出按钮，两者平时隐藏、鼠标移入标题栏时同步浮现；修复退出按钮点击无响应的问题。
- 修复 Home 页初始化期间播放页“简”按钮短暂闪现的问题，页面第一帧即使用正确的 Home 状态。
- 将极简模式渲染端逻辑拆分为独立模块，统一视觉状态保存、失败回滚和着色器同步；主进程仅在窗口切换成功后提交模式状态，并新增无依赖回归检查。
- 修复极简模式污染播放页歌词大小和镜头绑定设置的问题，极简歌词布局改为渲染时独立计算；退出放大窗口时短暂遮蔽 Chromium 旧尺寸表面，消除左上角闪帧。
- 修复极简模式与大屏模式切换时的窗口定位闪烁问题，在退出极简模式时加入60ms的旧帧以隐藏窗口定位闪烁问题

## Daine优化版2.1.0

- 将前端代码按核心状态、播放、界面、系统与视觉模块进行工程化拆分，降低单文件维护成本。
- 修复 Home 页面与播放页 3D 歌单架的交叉交互和事件穿透问题。
- 删除左侧大卡片的天气系统，改为展示每日热门歌曲中的副歌歌词。
- 新增左侧大卡片自定义背景功能。
- 启动界面新增 `Daine优化版` 标识，沿用 Mineradio 字体与动画质感，作为 Mine / radio 之后的第三段呈现。
- Home 页面交互优化：移除点击空白处隐藏 Home 的逻辑，左侧大卡片不再因底部控制器出现而缩小，控制器改为覆盖在更上层。
- Home 右侧六张卡片改为中文入口：`我喜欢`、`今日推荐`、`上次听到`、`热门歌单`、`我的播客`、`我的歌单`，并重新匹配对应播放与列表功能。
- 新增 `我喜欢` 专属歌单视图：支持网易云 / QQ / 酷狗平台筛选，未登录平台会给出明确提示；打开后直接展开对应平台的红心歌单歌曲列表。
- 优化 `我喜欢` 专属列表视觉：取消重复歌单卡片，改为顶部歌单信息加下方歌曲大列表，并修复滚动条挤压卡片和圆角边框显色问题。
- Search 区域新增音乐平台今日热门入口，热门歌曲名可直接填入搜索框并触发搜索，列表高度限制在画面可控范围内。
- 视觉控制台清理：移除 FX panel 控制台里的“鼠标移开自动隐藏”提示，适配新的关闭窗口逻辑。
