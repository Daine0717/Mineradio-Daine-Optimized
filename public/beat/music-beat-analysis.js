...
async function fetchBeatPrefetchAudioUrl(song) {
  ...
  if (provider === 'kugou') {
    var kgQuality = normalizePlaybackQuality(playbackQuality);
    var kg = await apiJson('/api/kugou/song/url?hash=' + encodeURIComponent(song.hash || song.id || '') + '&albumAudioId=' + encodeURIComponent(song.albumAudioId || song.album_audio_id || '') + '&albumId=' + encodeURIComponent(song.albumId || song.album_id || '') + '&quality=' + encodeURIComponent(kgQuality) + kugouQualityHashesParam(song));
    if (!kg || !kg.url) return null;
    return '/api/audio?url=' + encodeURIComponent(kg.url);
  }
