...
(function initDesktopWindowShell(){
  ...
  var minimalBtns = document.querySelectorAll('[data-window-action="minimal"]');
  ...
  function applyState(state) {
    ...
    if (!minimalModeBusy) document.body.classList.toggle('minimal-mode', !!desktopWindowState.isMinimalMode);
    if (!minimalModeBusy) setMinimalShaderMode(!!desktopWindowState.isMinimalMode);
    ...
    minimalBtns.forEach(function(btn){
      btn.title = desktopWindowState.isMinimalMode ? '退出极简模式' : '极简模式';
      btn.setAttribute('aria-label', btn.title);
      btn.setAttribute('aria-pressed', desktopWindowState.isMinimalMode ? 'true' : 'false');
    });
  }
  ...
  document.querySelectorAll('[data-window-action]').forEach(function(btn){
    btn.addEventListener('click', function(e){
      ...
      if (action === 'minimal') toggleMinimalMode();
  ...
  document.addEventListener('dblclick', function(e){
    if (!document.body.classList.contains('minimal-mode') || e.target.closest('button')) return;
    e.preventDefault();
    e.stopImmediatePropagation();
    setMinimalMode(false);
  }, true);
  document.addEventListener('keydown', function(e){
    if (e.key === 'Escape' && document.body.classList.contains('minimal-mode')) {
      e.preventDefault();
      setMinimalMode(false);
    }
  }, true);
