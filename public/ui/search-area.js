function requestSearchPillGlassUpdate() {
...
function renderSearchHotSection() {
  if (!searchHotItems.length) {
    return '<div class="search-hot"><div class="search-history-head"><span>今日热门</span></div><div class="search-empty">' + (searchHotLoading ? '正在整理热门歌曲' : '暂无热门歌曲') + '</div></div>';
  }
  return '<div class="search-hot">' +
    '<div class="search-history-head"><span>音乐平台今日热门</span></div>' +
    '<div class="search-hot-list">' +
      searchHotItems.map(function(q){ return '<button class="search-hot-item" type="button" data-hot-query="' + escHtml(q) + '">' + escHtml(q) + '</button>'; }).join('') +
    '</div>' +
  '</div>';
}
async function loadSearchHot() {
  if (searchHotLoading || searchHotItems.length) return;
  searchHotLoading = true;
  try {
    var data = await apiJson('/api/music/hot?limit=12');
    searchHotItems = (data && data.hot || []).map(function(q){ return String(q || '').trim(); }).filter(Boolean).slice(0, 12);
  } catch (err) {
    console.warn('Music hot:', err);
  }
  searchHotLoading = false;
  if ($input && !$input.value.trim() && isMusicSearchMode(searchMode)) renderSearchHistory();
}
function renderSearchHistory() {
  if (searchMode !== 'song') return false;
  var items = readSearchHistory();
  if (!searchHotItems.length && !searchHotLoading) loadSearchHot();
  $results.innerHTML =
    (items.length ? (
    ...
    renderSearchHotSection();
  $results.classList.add('show');
  requestSearchPillGlassUpdate();
  return true;
}
...
$results.addEventListener('click', function(e){
  ...
  var hotItem = e.target && e.target.closest ? e.target.closest('[data-hot-query]') : null;
  if (hotItem) {
    e.preventDefault();
    e.stopPropagation();
    runSearchHistory(hotItem.getAttribute('data-hot-query') || '');
  }
});
