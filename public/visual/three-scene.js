...
function getRenderPixelRatio() {
  ...
  var viewport = typeof mainRenderViewportSize === 'function' ? mainRenderViewportSize() : { width:innerWidth, height:innerHeight };
  ...
  if (document.body.classList.contains('minimal-mode')) cap = Math.min(cap, 1);
  return Math.max(quality.min, Math.min(device, cap));
}
function getRenderPixelLoad() {
  var ratio = getRenderPixelRatio();
  var viewport = typeof mainRenderViewportSize === 'function' ? mainRenderViewportSize() : { width:innerWidth, height:innerHeight };
  return Math.max(1, viewport.width * viewport.height) * ratio * ratio;
}
