...
function lyricCameraLockFit(layoutScale, layoutX, layoutY, distance) {
  ...
  if (document.body.classList.contains('minimal-mode')) {
    var renderHeight = typeof mainRenderViewportSize === 'function' ? mainRenderViewportSize().height : Math.max(1, innerHeight);
    var croppedHeight = visibleH * Math.min(1, innerHeight / renderHeight) * 0.58;
    var widthFit = visibleW * 0.80 / Math.max(0.01, bounds.w * layoutScale);
    var heightFit = croppedHeight / Math.max(0.01, bounds.h * layoutScale);
    return clampRange(Math.min(1, widthFit, heightFit), 0.52, 1);
  }
...
window.addEventListener('mineradio-resize-end', repositionFxFloatingPanels);
...
function updateStageLyrics3D(dt) {
  ...
  var minimalLyricLayout = document.body.classList.contains('minimal-mode');
  var layoutScale = minimalLyricLayout ? 1 : clampRange(Number(fx.lyricScale) || 1, 0.35, 1.65);
  var layoutX = minimalLyricLayout ? 0 : clampRange(Number(fx.lyricOffsetX) || 0, -2.0, 2.0);
  var layoutY = minimalLyricLayout ? 0 : clampRange(Number(fx.lyricOffsetY) || 0, -1.2, 1.35);
  var layoutZ = minimalLyricLayout ? 0 : clampRange(Number(fx.lyricOffsetZ) || 0, -1.6, 1.6);
  var layoutTiltX = minimalLyricLayout ? 0 : clampRange(Number(fx.lyricTiltX) || 0, -42, 42);
  var layoutTiltY = minimalLyricLayout ? 0 : clampRange(Number(fx.lyricTiltY) || 0, -42, 42);
  ...
  var cameraLockedLyrics = (minimalLyricLayout || fx.lyricCameraLock || wallpaperLyricLock) && camera;
  ...
  if (!cameraLockedLyrics && !skullLyricEdgeGuard && !skullMouthLyrics) stageLyrics.lockFitScale = 1;
  else {
  ...
  function tickMesh(mesh, isCurrent) {
    ...
    if (isCurrent) {
      ...
      if (skullMouthLyrics) {
        ...
        var minimalLyric = document.body.classList.contains('minimal-mode');
        var lyricCenterY = minimalLyric ? 0 : 0.18;
        var lyricFloatY = Math.sin(t * 0.55 + seed) * (minimalLyric ? 0.012 : 0.055) + Math.sin(t * 1.35 + seed) * (minimalLyric ? 0.004 : 0.014);
        mesh.position.y += ((lyricCenterY + lyricFloatY) - mesh.position.y) * (minimalLyric ? 0.18 : 0.075);
...
function tickLyricsParticles() {
  ...
  if ((!playing && !document.body.classList.contains('minimal-mode')) || !audio || !lyricsLines.length) {
