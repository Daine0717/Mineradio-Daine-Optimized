...
async function handleMusicHot(limit) {
  const fallback = ['APT.', '跳楼机', '若月亮没来', 'Die With A Smile', '悬溺', '凄美地', '晴天', '告白气球'];
  try {
    const r = await search_hot_detail({ cookie: userCookie, timestamp: Date.now() });
    const raw = r && r.body && r.body.data;
    const names = (Array.isArray(raw) ? raw : [])
      .map(item => String(item && (item.searchWord || item.name || item.keyword) || '').trim())
      .filter(Boolean);
    return { provider: 'netease', hot: names.slice(0, limit) };
  } catch (err) {
    console.warn('[MusicHot]', err.message);
    return { provider: 'fallback', hot: fallback.slice(0, limit) };
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/music/hot') {
    try {
      const limit = Math.max(6, Math.min(20, parseInt(url.searchParams.get('limit') || '12', 10) || 12));
      sendJSON(res, await handleMusicHot(limit));
    } catch (err) {
      console.error('[MusicHot]', err);
      sendJSON(res, { provider: 'fallback', hot: [] }, 500);
    }
    return;
  }
